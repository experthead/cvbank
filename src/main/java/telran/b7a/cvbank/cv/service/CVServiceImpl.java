...
public class CVServiceImpl implements CVService {
	...
	public List<CVDto> getCVsBySalary(int minSalary, int maxSalary) {
		return cvRepository.findCVsBySalaryBetween(minSalary, maxSalary)
					.map(cv -> modelMapper.map(cv, CVDto.class)).toList();
	}

	@Override
	public List<CVDto> getCVsByPosition(String position) {
		return cvRepository.findCVsByPosition(position)
				.map(cv -> modelMapper.map(cv, CVDto.class)).toList();
	}

	@Override
	public List<CVDto> getCVsBySkills(Set<String> skills) {
		return cvRepository.findCVsBySkills(skills)
				.map(cv -> modelMapper.map(cv, CVDto.class)).toList();
	}
